...
func _process(delta):
	...
	match global.player_turn:
		...
		2:
			if player_area.name == "player_area2" && global.can_select:
				ai_move_func()
		3:
			if player_area.name == "player_area3" && global.can_select:
				ai_move_func()
		4:
			if player_area.name == "player_area4"  && global.can_select:
				ai_move_func()
...
func select_card(player_cards):
	player_cards.erase(player_cards[int(self.name.substr(4)) - 1])
...
func ai_move_func():
			...
			#print(selected_card + "       " + str(global.player_turn) + "        " + str(global.start_turn))
